// Dashboard data: one fetch, stats per metric. Used by the page (views only) and the lazy metric action.
import { compact, METRICS, pct, plain } from '@/components/dashboard/format'
import type { PerMetric, TopVideo } from '@/components/dashboard/dashboard-client'
...
export const ALL_METRICS = Object.keys(METRICS) as Metric[]
...
export async function loadDashboard(supabase: Awaited<ReturnType<typeof createClient>>, metrics: Metric[]) {
  const [settingsQ, baseQ, dimsQ, pairsQ, scoresQ, topQ, videoCountQ] = await Promise.all([
  ...
  const pairsFor: Partial<Record<Metric, ReturnType<typeof pairStats>>> = {}
  const perMetric = (metric: Metric): PerMetric => {
    const singles = labelStats(dims.filter((r) => r.metric === metric), base)
    const pairs = (pairsFor[metric] = pairStats(pairRows.filter((r) => r.metric === metric), base))
  ...
  const data = Object.fromEntries(metrics.map((m) => [m, perMetric(m)])) as Record<Metric, PerMetric>

  return { data, settings, base, pairsFor, p0 }
}
